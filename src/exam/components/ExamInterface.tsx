...
const initializeSections = (sections: Section[]): Section[] => {
  const shuffled = sections.map((section) => ({
    ...section,
    questions: (section.shuffleQuestions ? shuffleArray(section.questions) : section.questions)
      .map((q) => q.shuffleOptions && q.options && q.options.length > 1
...
export const ExamInterface: React.FC<ExamInterfaceProps> = ({
  examData, studentName, studentEmail = '', sessionKey,
  jwtToken = '', isJwtMode = false,
  onSubmit, onViolation, onSuppressViolations, onPhaseActive, violations,
}) => {
  ...
  useEffect(() => {
    if (allQuestions.length > 0 && questionStatuses.length === 0) {
      setQuestionStatuses(allQuestions.map((q) => ({ questionId: q.id, status: 'not-visited' })));
    }
  ...
  useEffect(() => {
    if (currentQuestion && examPhase === 'active') {
      setQuestionStatuses((prev) =>
        prev.map((s) => s.questionId === currentQuestion.id && s.status === 'not-visited'
          ? { ...s, status: 'not-answered' } : s)
      );
    }
  }, [currentQuestionIndex, examPhase]);

  useEffect(() => { return () => { stopAllRecording(); }; }, []);

  useEffect(() => {
    if (screenStatus === 'stopped' && recording.screen) {
      setScreenReady(false);
      if (examPhase === 'active') onViolation?.();
    }
  }, [screenStatus]);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setTimeRemaining((prev) => {
        if (prev <= 1) { clearInterval(timer); doAutoSubmitRef.current(); return 0; }
        return prev - 1;
      });
    }, 1000);
    ...
    const interval = setInterval(() => {
      ...
      if (remaining <= 1) {
        ...
        setCurrentQuestionIndex(prev => {
          ...
          return prev < qs.length - 1 ? prev + 1 : prev;
        });
      } else {
  ...
  const handleAnswerChange = (answer: string | string[]) => {
    setAnswers((prev) => {
      ...
      if (existing) return prev.map((a) => a.questionId === currentQuestion.id ? { ...a, answer } : a);
  ...
  const handleMarkToggle = () => {
    setAnswers((prev) => {
      const existing = prev.find((a) => a.questionId === currentQuestion.id);
      if (existing) return prev.map((a) => a.questionId === currentQuestion.id ? { ...a, isMarked: !a.isMarked } : a);
      ...
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-lg w-full">
          <div className="mb-6">
            ...
            <p className="text-gray-500 text-sm">Complete the setup below, then click <strong>Begin Exam</strong>.</p>
          </div>
          <div className="space-y-4 mb-8">
            {recording.camera && (
              <div className={`flex items-center gap-4 p-4 rounded-lg border-2 transition ${cameraReady ? 'border-green-400 bg-green-50' : 'border-gray-200'}`}>
                <div className={`w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center ${cameraReady ? 'bg-green-500' : 'bg-gray-300'}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-500 truncate">{cameraReady ? 'Active' : cameraError ? `Error: ${cameraError}` : 'Click Grant to allow access'}</p>
                </div>
                {!cameraReady && (
                  <button onClick={handleStartCamera} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 whitespace-nowrap">
                    {cameraError ? 'Retry' : 'Grant'}
                  </button>
                )}
                {cameraReady && <svg className="w-6 h-6 text-green-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>}
              </div>
            )}
            {recording.screen && (
              <div className={`flex items-center gap-4 p-4 rounded-lg border-2 transition ${screenReady ? 'border-green-400 bg-green-50' : 'border-gray-200'}`}>
                <div className={`w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center ${screenReady ? 'bg-green-500' : 'bg-gray-300'}`}>
                  <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-500 truncate">{screenReady ? 'Sharing entire screen' : screenError ? screenError : 'You must share your entire screen'}</p>
                </div>
                {!screenReady && (
                  <button onClick={handleStartScreenShare} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 whitespace-nowrap">
                    {screenError ? 'Retry' : 'Share Screen'}
                  </button>
                )}
                {screenReady && <svg className="w-6 h-6 text-green-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>}
              </div>
            )}
          </div>
          <button onClick={() => setExamPhase('disclaimer')} disabled={!allReady}
          ...
          {!allReady && <p className="text-center text-xs text-gray-400 mt-3">Complete all recording steps before starting.</p>}
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center z-[9999] p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full text-center">
          ...
          <h2 className="text-xl font-bold text-gray-800 mb-2">{examPhase === 'active' ? 'Exam Paused' : 'Screen Share Required'}</h2>
          <p className="text-gray-500 text-sm mb-6">Screen sharing has stopped. You must share your entire screen to {examPhase === 'active' ? 'resume the exam.' : 'continue.'}</p>
          {screenError && <p className="text-red-500 text-xs mb-4 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{screenError}</p>}
          <button onClick={handleStartScreenShare} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-xl transition text-sm">Share Screen</button>
        </div>
      </div>
    );
  }
  ...
  if (examPhase === 'disclaimer') {
    ...
    const rules = [
      ...
      { title: 'No Page Refresh', desc: 'Refreshing the page (F5 / Ctrl+R) is blocked.' },
      ...
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-lg w-full max-h-screen overflow-y-auto">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 mb-5 text-sm text-blue-800">
            ...
            <span className="mx-2 text-blue-300">|</span>Code: {examData.examCode}
            <span className="mx-2 text-blue-300">|</span>Duration: {Math.floor(examData.duration / 60)} min
          </div>
          ...
          {questionLoadError && (
            <p className="mb-4 px-4 py-3 bg-red-50 border border-red-200 text-red-700 rounded-lg text-sm">{questionLoadError}</p>
          )}
  ...
  return (
    <div className="min-h-screen bg-gray-50 relative">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[9999] p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center">
            ...
            <p className="text-gray-500 text-sm mb-1">
              You have answered <span className="font-semibold text-gray-700">
                {answers.filter(a => { const ans = a.answer; return Array.isArray(ans) ? ans.length > 0 : ans !== ''; }).length}
              </span> of <span className="font-semibold text-gray-700">{allQuestions.length}</span> questions.
            </p>
            <p className="text-gray-400 text-xs mb-6">This action cannot be undone.</p>
            <div className="flex gap-3">
              <button onClick={() => setShowSubmitModal(false)} className="flex-1 py-3 border border-gray-300 text-gray-700 rounded-xl font-medium hover:bg-gray-50 transition text-sm">Cancel</button>
              <button onClick={handleConfirmSubmit} className="flex-1 py-3 bg-green-600 hover:bg-green-700 text-white rounded-xl font-semibold transition text-sm">Submit</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="fixed inset-0 pointer-events-none z-0 flex items-center justify-center"
        style={{ fontSize: '120px', color: 'rgba(0,0,0,0.03)', fontWeight: 'bold', transform: 'rotate(-45deg)', userSelect: 'none' }}>
        {studentName}
      </div>
      ...
      <div className="relative z-10">
        ...
        <div className="max-w-7xl mx-auto p-4 grid grid-cols-1 lg:grid-cols-4 gap-4">
          <div className="lg:col-span-3 space-y-4">
            {currentQTimerDisplay !== null && (
              <div className={`flex items-center justify-between px-4 py-2.5 rounded-lg border text-sm font-medium ${
                currentQTimerDisplay <= 10 ? 'bg-red-50 border-red-300 text-red-700 animate-pulse'
                : currentQTimerDisplay <= 30 ? 'bg-amber-50 border-amber-300 text-amber-700'
                : 'bg-blue-50 border-blue-200 text-blue-700'}`}>
                <span>⏱ Question Time</span>
                <span className="font-bold tabular-nums text-base">
                  {formatTime(currentQTimerDisplay)}
                  {currentQTimerDisplay <= 10 && <span className="ml-2 text-xs font-normal">auto-advancing…</span>}
                </span>
              </div>
            )}
            ...
            {currentQuestion && (
              <QuestionDisplay
                ...
                onAnswerChange={handleAnswerChange} onMarkToggle={handleMarkToggle}
                sectionName={currentQuestion.sectionName} locked={timedOutQuestions.has(currentQuestion.id)}
              />
            )}

            <div className="bg-white rounded-lg p-4 flex items-center justify-between">
              <button onClick={handlePrevious} disabled={currentQuestionIndex === 0 || !examData.canNavigate}
          ...
          <div className={`${showNavigator ? 'block' : 'hidden'} lg:block`}>
            <QuestionNavigator
              examData={examData} sections={activeSections}
              currentQuestionIndex={currentQuestionIndex} answers={answers}
              questionStatuses={questionStatuses} onNavigate={setCurrentQuestionIndex}
