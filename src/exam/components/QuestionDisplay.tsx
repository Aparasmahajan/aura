...
export const QuestionDisplay: React.FC<QuestionDisplayProps> = ({
  question, answer, onAnswerChange, onMarkToggle, sectionName, locked = false,
}) => {
    ...
    <div className={`bg-white rounded-lg p-6 space-y-6 ${locked ? 'opacity-80' : ''}`}>
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="text-sm text-gray-500 mb-2">Section {sectionName} | Question {question.number}</div>
          ...
          {question.options.map((option) => (
            ...
            >
              ...
              {option.type === 'text' ? (
                ...
                <img src={option.text} alt={`Option ${option.id}`} className="max-w-xs max-h-48 object-contain rounded" />
              )}
            </label>
          ))}
        </div>
