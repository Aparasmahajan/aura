...
export const QuestionNavigator: React.FC<QuestionNavigatorProps> = ({
  sections, currentQuestionIndex, answers, questionStatuses, onNavigate, canNavigate,
}) => {
  ...
  const stats = {
    answered:   questionStatuses.filter((s) => { const a = answers.find((x) => x.questionId === s.questionId); return hasContent(a) && !a?.isMarked; }).length,
    notAnswered: questionStatuses.filter((s) => { const a = answers.find((x) => x.questionId === s.questionId); return s.status === 'not-answered' && !hasContent(a); }).length,
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-lg max-h-[80vh] overflow-y-auto">
      ...
      <div className="grid grid-cols-2 gap-2 mb-4 text-sm">
        <div className="flex items-center gap-2"><div className="w-4 h-4 bg-green-500 rounded" /><span>Answered: {stats.answered}</span></div>
        <div className="flex items-center gap-2"><div className="w-4 h-4 bg-yellow-500 rounded" /><span>Not Answered: {stats.notAnswered}</span></div>
        <div className="flex items-center gap-2"><div className="w-4 h-4 bg-purple-500 rounded" /><span>Marked: {stats.marked}</span></div>
        <div className="flex items-center gap-2"><div className="w-4 h-4 bg-gray-200 rounded" /><span>Not Visited: {stats.notVisited}</span></div>
      </div>
            ...
            {section.questions.map((question) => {
              ...
              return (
                <button
                  ...
                  disabled={!clickable}
                  className={`p-2 rounded text-sm font-medium transition ${getStatusColor(status)} ${isCurrent ? 'ring-2 ring-blue-600 ring-offset-2' : ''} ${clickable ? 'hover:opacity-80 cursor-pointer' : 'cursor-not-allowed opacity-60'}`}
                >
                  {question.number}
                </button>
              );
